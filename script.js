...
function loadState() {
  ...
  if (hash && hash.startsWith('#state=')) {
    try {
      ...
      if (state.eventName) eventNameInput.value = state.eventName;
      teamCountInput.value = state.count;
      ...
      // Remove hash from URL so it doesn't persist on explicit refresh
      window.history.replaceState(null, null, window.location.pathname);
      return;
    } catch(err) {
      console.error('Share link decode error:', err);
      showError('Failed to load shared tournament link. It may be broken or too large.');
    }
  }
  ...
  if (saved) {
    try {
      ...
      if (state.eventName && eventNameInput) eventNameInput.value = state.eventName;
      if (teamCountInput) teamCountInput.value = state.count;
      bracketRounds = state.rounds;
      currentMatches = state.matches;
      
      const pubTitle = document.getElementById('public-title');
      if (pubTitle && state.eventName) pubTitle.textContent = state.eventName;

      // Auto-skip to bracket view
      if (teamInputCard) teamInputCard.style.display = 'none';
      const emptyState = document.getElementById('empty-state');
      if (emptyState) emptyState.style.display = 'none';
      
      renderBracket();
      showToast('Restored saved tournament state!');
    } catch(err) {
      console.error(err);
      if (localStorage.getItem('valTournamentState')) {
        localStorage.removeItem('valTournamentState');
      }
    }
  } else {
    // If no state exists and we are on the public viewer, reset it
    const emptyState = document.getElementById('empty-state');
    if (emptyState) {
...
// React instantly to cross-tab updates (Admin updating storage creates a magical live-update effect here!)
window.addEventListener('storage', (e) => {
  if (e.key === 'valTournamentState') {
    loadState();
  }
});
